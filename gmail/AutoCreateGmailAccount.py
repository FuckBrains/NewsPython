...
class CreateGmail:
    ...
    def Initialize(self):
        self._browser = webdriver.Chrome(executable_path=os.path.join(os.getcwd(), 'chromedriver'))
    ...
    def CreateAccount(self):
        ...
        print("----------------------")
        self._browser.find_element_by_id('firstName').click();
        self._browser.find_element_by_id('firstName').click();
        self._browser.find_element_by_id('firstName').send_keys(self._firstname)
        time.sleep(1)
        self._browser.find_element_by_id("lastName").send_keys(self._lastname)
        time.sleep(1)
        self._browser.find_element_by_id("username").send_keys(self._username)
        time.sleep(1)
        self._browser.find_element_by_xpath("//input[@name='Passwd']").send_keys(self._pswd)
        time.sleep(1 + 3 * random.random())
        self._browser.find_element_by_xpath("//input[@name='ConfirmPasswd']").send_keys(
            self._pswd
        )
        self._browser.find_element_by_xpath("//span[text()='Next']").click()
